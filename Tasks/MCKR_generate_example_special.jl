# The MCKR generate example special task.
...
include(joinpath(pwd(),"Temp","MCKR_variables_upper_bound_dict.jl"))
...
@fastmath function sumo(B,NN)
    ...
    COEFF_string=""
    for i=1:Number_of_variables
        COEFF_string=COEFF_string*"$(Variables_upper_bound_dict[i])*"
    end
    ...
    for i=1:Number_of_variables
        write(Example_file,"    x$i=samplor(0,$(Variables_upper_bound_dict[i]))\n")
    end
...
@fastmath function sumo_antithetic(B,NN)
    ...
    centero_string="["
    for i=1:Number_of_variables
        centero_string=centero_string*"(0+$(Variables_upper_bound_dict[i]))/2,"
    end
    for i=Number_of_variables+1:Number_of_parameters
